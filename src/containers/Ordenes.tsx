...
import { OrdenesStatus, tdsOrdenesCap, tdsOrdenesFd } from '../types/OrdenesTypes';
import { RootState } from '../reducers/rootReducer';
import { getDsOrdenesRequest, getDsOrdenesReset } from '../actions/ordenesAction';
...
import { GetOrdenesCapEstatusState, TdsOrdenesCap, TdsOrdenesCapPend } from '../types/GetOrdenesCapEstatusType';
...
const Ordenes: React.FC<AllProps> = ({ ordenes, getOrdenesCapEstatusRespuesta, loginObject, catalogoEmisorasRespuesta, ordenesCapEnIssuerProfile, volverACargarApiInfoEmisorasEnIssuerProfile, seHizoRefreshInfoEmis = false }) =>{
    ...
    const [ todasOrdenesCapPendOCE, setTodasOrdenesCapPendOCE] = useState<TdsOrdenesCapPend[]>([]);
    ...
    useEffect(() => {
        if(ordenesCapEnIssuerProfile === undefined){
            /*
                este componente no esta en issuerProfile, entonces sacamos las 
                ordenes usando el api de ordenes (que trae todas las ordenes), y 
                el objeto de login.
            */
           ////console.log("ordenes ordenesCapEnIssuerProfile === undefined");
                ...
                if(loginObject.response.ierror === -1){
                    if(loginObject.response.dsLogin.tdsLogin.length > 0){
                        ...
                        if(cuentaSesionLO != 0){
                            ...
                            /*
                                por mientras comentamos este dispatch porque no usaremos la 
                                respuesta; cuando usemos la respuesta, lo descomentamos:D
                                
                                message = "/consulta/ordenescapestatus?cuenta=" + cuentaLO + "&portafolio=false";
                                dispatch(getOrdenesCapEstatusRequest({message, params: [params[1], params[2], params[3], params[4]]}));
                            */
                        }
                    }
                    else{
            ...
            /*
                este componente esta en IssuerProfile, y ya tiene las 
                ordenes que se mostraran en el componente, entonces solo 
                las agregamos al hook de ordenesCap.
            */
            ////console.log("ordenes ordenesCapEnIssuerProfile !== undefined");
            ////console.log(ordenesCapEnIssuerProfile);
            sendTodasLasOrdenesCap(ordenesCapEnIssuerProfile);
        }

        //para cuando el componente se unmountee (cuando cambias de pag por ejemplo, o el componente se cierra)
        /*
            CORREGIR este bug:
                cuando el componente esta en IssuerProfile, se desmonta por 
                alguna razon, cuando se cierran los modales de 
                editar o cancelar orden, hay que corregir esto!!!!
        */
        return () => {
            //reseteamos el estado de ordenes
            //console.log("reset componente ordenes");
            //setSeHizoRefreshCap(false);
            //setSeHizoRefreshFd(false);
            sendTodasLasOrdenesCap([]);
            sendTodasLasOrdenesFd([]);
            dispatch(getDsOrdenesReset({ hacerResetDelEstado: true }));
        }
    },[]);
    ...
    useEffect(() => {
        if(ordenesCapEnIssuerProfile === undefined){
            if(getOrdenesCapEstatusRespuesta !== undefined){
                if(getOrdenesCapEstatusRespuesta.message.length > 0 && getOrdenesCapEstatusRespuesta.loading === false){
                    // ya llego la respuesta del api
                    if(getOrdenesCapEstatusRespuesta !== undefined){
                        if(getOrdenesCapEstatusRespuesta.getOrdenesCapEstatusRespuesta !== undefined){
                            if(getOrdenesCapEstatusRespuesta.getOrdenesCapEstatusRespuesta.dsOrdenesCap !== undefined){
                                if(getOrdenesCapEstatusRespuesta.getOrdenesCapEstatusRespuesta.dsOrdenesCap.dsOrdenesCap !== undefined){
                                    if(getOrdenesCapEstatusRespuesta.getOrdenesCapEstatusRespuesta.dsOrdenesCap.dsOrdenesCap.tdsOrdenesCap !== undefined){
                                        sendTodasOrdenesCapOCE(getOrdenesCapEstatusRespuesta.getOrdenesCapEstatusRespuesta.dsOrdenesCap.dsOrdenesCap.tdsOrdenesCap);
                                    }
                                }
                            }// if dsOrdenesCap !== undefined

                            if(getOrdenesCapEstatusRespuesta.getOrdenesCapEstatusRespuesta.dsOrdenesCapPend !== undefined){
                                if(getOrdenesCapEstatusRespuesta.getOrdenesCapEstatusRespuesta.dsOrdenesCapPend.dsOrdenesCapPend !== undefined){
                                    if(getOrdenesCapEstatusRespuesta.getOrdenesCapEstatusRespuesta.dsOrdenesCapPend.dsOrdenesCapPend.tdsOrdenesCapPend !== undefined){
                                        sendTodasOrdenesCapPendOCE(getOrdenesCapEstatusRespuesta.getOrdenesCapEstatusRespuesta.dsOrdenesCapPend.dsOrdenesCapPend.tdsOrdenesCapPend);
                                    }
                                }
    ...
    }, [getOrdenesCapEstatusRespuesta?.message, getOrdenesCapEstatusRespuesta?.loading]);
    ...
    const volverCargarApiOrdenes = (volvemosCargarApiOrdenes: boolean = true) => {
        ...
        if(ordenesCapEnIssuerProfile !== undefined && volvemosCargarApiOrdenes === false){
            volverACargarApiInfoEmisorasEnIssuerProfile && volverACargarApiInfoEmisorasEnIssuerProfile();
        }
    ...
    const DropdownDataOrdenesCapMostrar = todasLasOrdenesCap.length <= 5 ?
            [
                {
                    id: "op1",
    ...
    const [opCapMostrar, setOpCapMostrar] = useState(DropdownDataOrdenesCapMostrar[0].option);
    const [ordenesCapMostrar, setOrdenesCapMostrar] = useState<tdsOrdenesCap[]>([]);
    const [dropdownMostrarCapOpen, setDropdownMostrarCapOpen] = useState<boolean>(false);
    ...
    const DropdownDataOrdenesFdMostrar = todasLasOrdenesFd.length <= 5 ?
        [
            {
                id: "op1",
    ...
    const [opFdMostrar, setOpFdMostrar] = useState(DropdownDataOrdenesFdMostrar[0].option);
    const [ordenesFdMostrar, setOrdenesFdMostrar] = useState<tdsOrdenesFd[]>([]);
    const [dropdownMostrarFdOpen, setDropdownMostrarFdOpen] = useState<boolean>(false);
        ...
        return (
            <li className="flex-auto text-center">
                <a
                    ...
                    // }
                    className={ ("inline-block list-none py-2 border-transparent border-b-2 hover:border-red-600 hover:text-red-600 cursor-pointer ") + (index === 0 ? " px-2 " : " px-4 ") + ( index === openTab ? " text-red-600 border-red-600 " : "")}
                    onClick={(e) => {
                        e.preventDefault();
                        setOpenTab(index);
                    }}
                ...
                <>
                    {
                        ...
                        : ordenesCapMostrar.length > 0 ?
                            <>
                                <p className="text-xs text-gray-500 pb-2 text-right">Mostrando {ordenesCapMostrar.length.toString()} de {todasLasOrdenesCap.length.toString()} órdenes.</p>
                                {
                                    /*
                                    ...
                                    ordenesCapMostrar.map((node:tdsOrdenesCap)=>{
                                        ...
                                        if(catalogoEmisorasRespuesta !== undefined){
                                            if(catalogoEmisorasRespuesta.catalogoEmisorasRespuesta !== undefined){
                                                if(catalogoEmisorasRespuesta.catalogoEmisorasRespuesta.length > 0){
                                                    const catalogoFilter = catalogoEmisorasRespuesta.catalogoEmisorasRespuesta.filter((item:Emisora)=>{
                                                        return item.Emisora+"."+item.Serie === node.emisora
                                                    });

                                                    if( catalogoFilter !== undefined && catalogoFilter?.length > 0 ){
                                        ...
                                        return (
                                            <OrdenDeCapitales 
                                                ...
                                                volverCargarApiOrdenes={() => volverCargarApiOrdenes(ordenesCapEnIssuerProfile !== undefined ? false : true)}
                                            />
                                        )
                                    })
                                }
                                    ...
                                    <div className="flex flex-row justify-end items-center mt-2">
                                        ...
                                        >
                                            <DropdownTrading
                                                ...
                                                side={false}
                                                sendOption={(opMostrar:any) => sendOpCapMostrar(opMostrar)}
                                                openD = {dropdownMostrarCapOpen}
                                                //idDivDDTrading={IdOrdenesCapMostrarDD}
                                                sendOpenD={(newOpen: boolean) => sendDropdownMostrarCapOpen(newOpen)}
                                            />
                                        </div>
                                    </div>
                                }
                ...
                <>
                    {
                        ...
                        : ordenesFdMostrar.length > 0 ?
                            <>
                                <p className="text-xs text-gray-500 pb-2 text-right">Mostrando {ordenesFdMostrar.length.toString()} de {todasLasOrdenesFd.length.toString()} órdenes.</p>
                                {
                                   ordenesFdMostrar.map((node:tdsOrdenesFd)=>{
                                        return (
                                            <div key={node.numFolio} className="flex w-full justify-between items-center pb-2 border-b-2 mt-2 hover:shadow-xl px-4 py-2">
                                                ...
                                                <div className="w-1/24">
                                                    <p className="font-semibold text-sm">{node.tipoTransaccion}</p>
                                                </div>
                                                ...
                                                <div className="w-2/12 text-right">
                                                    <p className="text-sm font-bold" style={{color:"#283f72"}}>{node.descEstado}</p>
                                                    <p className="text-gray-400 text-xs">Liq {node.FecAplic}</p>
                                                </div>
                                            </div>
                                        )
                                    })
                                    ...
                                    <div className="flex flex-row justify-end items-center mt-4">
                                        ...
                                        >
                                            <DropdownTrading
                                                ...
                                                side={false}
                                                sendOption={(opMostrar:any) => sendOpFdMostrar(opMostrar)}
                                                openD = {dropdownMostrarFdOpen}
                                                //idDivDDTrading={IdOrdenesFdMostrarDD}
                                                sendOpenD={(newOpen: boolean) => sendDropdownMostrarFdOpen(newOpen)}
                                            />
                                        </div>
                                    </div>
                                }
        ...
        <div className="w-12/12">
            <div className="mt-1">
                <div className="flex flex-row pb-2">
                    ...
                    <div className="mx-2">
                        <MdRefresh 
                            onClick={() => volverCargarApiOrdenes(ordenesCapEnIssuerProfile !== undefined ? false : true)} 
                            className="text-gray-500 text-lg hover:cursor-pointer hover:text-red-600" 
                        />
                    </div>
                </div>
                {
                    ordenesCapEnIssuerProfile !== undefined && <hr className="solid bg-gray-500 mb-3"/>
                }
                {
...
const mapDispatchToProps = (dispatch: any) => {
    return {
        ...
        getOrdenesCapEstatusRequest: () => dispatch(getOrdenesCapEstatusRequest(dispatch)),

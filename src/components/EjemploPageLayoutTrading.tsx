...
const EjemploPageLayoutTrading: React.FC<AllProps> = ({ loginObject, listItems, catalogoEmisorasRespuesta, portfolio, catalogoTrading, postList, moversList, catalogoListaEmisorasRespuesta, commodities, deleteIssuer, postIssuer, getPermisosRespuesta }) => {
            ...
            if(loginObject.response.ierror === -1){
                if(loginObject.response.dsLogin.tdsLogin.length > 0){
                    ...
                    if(cuentaSesionLO != 0){
                        ...
                        let message = "permisos/lista";
                        let params = [ canal, cuentaSesionLO.toString(), tokenLO,  idLO.toString() ];
                        dispatch(getPermisosRequest({ message, params }));
                        ...
                        if(portfolio.message == ""){
                            ...
                            message = "/consulta/portafolio?cuenta=" + cuentaLO.toString();
                            let params = ["", canal, cuentaSesionLO.toString(), tokenLO, idLO.toString()];
    ...
    useEffect(() => {
        if(getPermisosRespuesta !== undefined){
            if(getPermisosRespuesta.message.length > 0 && getPermisosRespuesta.loading === false){
                //ya llego la respuesta del api

                //revisamos si el servicio de tiempo real esta activo
                const filterServiciosTR = getPermisosRespuesta.getPermisosRespuesta.dsServicios.ttServicios.filter((servicio: TtServicios) => {
                    return servicio.Servicio.toLowerCase().trim() === "tiempo real";
                });

                if(filterServiciosTR.length > 0){
                    //si hay servicio de tiempo real, actualizamos el hook
                    sendServTiempoRealActivo(filterServiciosTR[0].Activo);
                }
            }
        }
    }, [getPermisosRespuesta.message, getPermisosRespuesta.loading]);
    ...
    /**
     * Constantes para la construccion del PageLayout
     */
    const styleSidebar = { width:"3.75%" };

    const classesContentPrincipal = "p-3";
    const styleContentPrincipal = { width: "74.25%" };
        ...
        <div className="flex flex-col">
            ...
            <div className="flex flex-col">
                <div className="searchGraficador my-4" style={{ height:"600px" }}>
                    <div className='py-2'>
                        <h1 className="font-sans text-lg text-gray-800  font-medium border-b-2 border-gray-300 pb-2">Graficador</h1>
                    </div>
                    <div className='flex'>
                ...
                <h1 className="font-sans text-lg text-gray-800 border-b border-gray-300 pb-2">Monitor de acciones</h1>
                <div className="my-4">
                ...
                <div className="my-4">
                    <h1 className="font-sans text-xl text-gray-800 font-medium pb-2 border-b border-gray-300 pb-2 mb-4 mt-4">Otras Emisoras</h1>
                    <CardPagination type={"similares"} />
                </div>
                <div className="my-4 container justify-around">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2">
                        ...
                        <div className="col-10">
                            <div>
                                <h1 className="font-sans text-xl text-gray-800 font-medium border-b border-gray-300 pb-2 mb-4 mt-4">Índices Fx</h1>    
                                {/* <IndiceFx/>  */}
                            </div>
                            <div>
...
const mapStateToProps = (store: RootState) => {
    return {
        ...
        getPermisosRespuesta: store.getPermisosRespuesta,
    };
};
...
const mapDispatchToProps = (dispatch: any) => {
    return {
        ...
        getPermisosRequest: () => dispatch(getPermisosRequest(dispatch)),
        getPermisosReset: () => dispatch(getPermisosReset(dispatch)),
    };
};

export default /*connect(mapStateToProps, mapDispatchToProps)*/(EjemploPageLayoutTrading);
